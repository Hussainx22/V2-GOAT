...
module.exports.onStart = async function ({ api, args, event, message }) {
  try {
    ...
    if (event.type === "message_reply") {
      const url = event.messageReply.attachments[0].url;
const wait = api.sendMessage("𝙋𝙧𝙤𝙘𝙚𝙨𝙨𝙞𝙣𝙜 𝙥𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩 🖤💫", event.threadID);
      try {
...
let seed
  ...
  const wait = message.reply("𝙋𝙧𝙤𝙘𝙚𝙨𝙨𝙞𝙣𝙜 𝙥𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩 🖤💫");

  try {
    const response = await axios.get(`https://nobs-api.onrender.com/dipto/genix?prompt=${encodeURIComponent(cleanedPrompt)}&ratio=${ratio}`);
    const link = response.data.data;
